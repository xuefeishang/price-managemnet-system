...
public interface PriceRepository extends JpaRepository<Price, Long> {
    ...
    @Query("SELECT p FROM Price p LEFT JOIN FETCH p.product pr LEFT JOIN FETCH pr.category WHERE p.product.id = :productId ORDER BY p.createdTime DESC LIMIT 1")
    Optional<Price> findFirstByProductIdOrderByCreatedTimeDesc(@Param("productId") Long productId);

    @Query("SELECT p FROM Price p LEFT JOIN FETCH p.product pr LEFT JOIN FETCH pr.category WHERE p.product.id = :productId ORDER BY p.createdTime DESC")
    List<Price> findByProductIdOrderByCreatedTimeDesc(@Param("productId") Long productId);

    /**
     * 按产品和生效日期精确查找价格记录（取最新创建的一条，防止重复数据导致异常）
     */
    @Query("SELECT p FROM Price p LEFT JOIN FETCH p.product pr LEFT JOIN FETCH pr.category WHERE p.product.id = :productId AND p.effectiveDate = :effectiveDate ORDER BY p.createdTime DESC LIMIT 1")
    Optional<Price> findFirstByProductIdAndEffectiveDateOrderByCreatedTimeDesc(@Param("productId") Long productId, @Param("effectiveDate") LocalDate effectiveDate);
     ...
     * 使用 LEFT JOIN FETCH 确保产品和分类关联被正确加载
     * 按product_id和createdTime排序，确保同产品同日期重复数据中最新记录被保留
     */
    @Query("SELECT DISTINCT p FROM Price p LEFT JOIN FETCH p.product pr LEFT JOIN FETCH pr.category WHERE p.effectiveDate = :date ORDER BY p.product.id, p.createdTime DESC")
     ...
     */
    @Query("SELECT p FROM Price p LEFT JOIN FETCH p.product pr LEFT JOIN FETCH pr.category WHERE p.product.id = :productId AND p.effectiveDate = :date ORDER BY p.createdTime DESC LIMIT 1")
     ...
     * 按product_id和createdTime排序，确保同产品同日期重复数据中最新记录被保留
     */
    @Query("SELECT DISTINCT p FROM Price p LEFT JOIN FETCH p.product pr LEFT JOIN FETCH pr.category WHERE p.product.id IN :productIds AND p.effectiveDate = :date ORDER BY p.product.id, p.createdTime DESC")
    ...
    @Override
    Optional<Price> findById(@Param("id") Long id);
}
