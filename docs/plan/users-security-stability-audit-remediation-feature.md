...
| 编号 | 风险等级 | 状态 | 审计发现 |
|---|---|---|---|
| A-01 | 高 | **已完成** | 未被任何客户端使用的公开注册接口 `/api/auth/register` 可绕过共享密码策略 |
| A-02 | 高 | **已完成** | 管理员用户更新接口和 Excel 用户导入可绕过共享密码策略 |
| A-03 | 中 | **已完成** | HTTP 拦截器和页面 `catch` 均可能显示错误 Toast，造成重复提示 |
| A-04 | 中 | **已完成** | 全局数据完整性异常日志只记录异常类型，无法定位约束冲突 |
| A-05 | 低 | **已完成** | 密码账号关联规则、角色写入失败事务回滚等关键分支缺少测试 |
...
- `[x]`：已完成并具备代码或自动化测试证据。
- `[ ]`：未完成、待执行或尚未完成环境验证。
- `部分完成`：主体已完成，但第二轮审计发现仍需继续收口。

| 整改批次 | 当前状态 | 说明 |
|---|---|---|
| 第一轮 A-01 至 A-05 | **主体已完成** | 公开注册关闭、受控 DTO、用户导入原子写入、单次 Toast、日志诊断和专项测试已实施 |
| 第一轮生产环境验证 | **未完成** | Flyway validate、生产配置核验和浏览器关键人工回归尚未执行 |
| 第二轮 B-01 至 B-06 | **已完成** | 代码、自动化测试和正式文档已完成；生产环境验证单列待办 |

### 已完成内容

- [x] 关闭公开注册入口及安全白名单。
- [x] 新增用户和用户更新使用受控 DTO。
- [x] 用户导入采用全量预检与单一事务原子写入，任意失败零写入。
- [x] 用户导入校验模板、字段、密码策略、文件内重复及数据库重复。
- [x] 用户导入成功、预检失败、并发冲突和系统异常记录真实响应码及安全日志摘要。
- [x] PC 用户管理错误请求最多显示一次 Toast。
- [x] 数据完整性异常日志仅输出安全约束标识和异常类型。
- [x] 后端完整测试和前端生产构建通过。
- [x] 第一轮实施记录已归档至 `docs/plan/done/users-security-stability-audit-remediation-feature.md`。
- [x] B-01：密码重置从 URL 查询参数改为 JSON 请求体。
- [x] B-02：创建、导入、角色分配和权限解析统一限制为启用角色。
- [x] B-03：管理员编辑资料与可选新密码改为单一事务。
- [x] B-04：实现 `deptId: null` 等可空字段的显式清空语义。
- [x] B-05：导入预检一次返回全部当前可发现错误并去重。
- [x] B-06：消除自动工号逐行查询和用户导入逐行 flush。

### 未完成内容

- [ ] 执行生产环境 Flyway validate、配置核验及浏览器关键人工回归。
...
- [x] 匿名注册入口已关闭。
- [x] 所有业务密码写入入口已逐项审查。
- [x] Java `UserUpdateRequest` 与 TypeScript `UpdateUserRequest` 一致。
- [x] 更新接口不能修改密码、角色和锁定状态。
- [x] 用户导入先全量校验模板、字段、文件内重复和数据库重复。
- [x] 任意导入预检错误均保证写入数量为 0。
- [x] Excel 明文密码和默认密码均执行共享策略。
- [x] 全量预检通过后的用户与角色写入位于单一事务，任意失败整批回滚。
- [ ] 用户导入完全消除逐行查询：显式用户名和工号已批量查询，自动工号生成仍逐行查询，转 B-06。
- [x] Java 与 TypeScript 用户导入结果和错误结构一致。
- [x] 用户与初始角色写入失败事务回滚测试通过。
- [x] 用户导入成功和失败均记录 `IMPORT` 操作日志及真实响应码。
- [x] IMPORT 操作日志仅包含安全统计、耗时和错误码摘要。
- [x] 用户导入通知不包含异常原文或个人敏感信息。
- [x] 批量用户名和显式工号 Repository 查询测试通过。
- [x] 用户相关 Entity、Flyway、`init.sql` 和数据字典一致性检查通过，本轮无结构变更。
- [x] 用户管理失败请求最多显示一次 Toast。
- [x] 完整性异常日志具备安全诊断信息且不泄露业务数据。
- [ ] 生产环境 Flyway validate 尚未执行。
- [ ] 生产配置 `show-sql=false` 和安全日志级别尚未完成环境核验。
...
公开注册入口：已关闭
已审查密码编码入口数量：5 类
允许的密码编码入口：均执行共享密码策略；初始化用户使用受控配置密码
用户更新受控 DTO：通过
Excel 模板和表头预检：通过
Excel 文件内重复预检：通过
Excel 数据库重复预检：通过
...
Excel 任意错误零写入：通过
Excel 整批事务回滚：通过
Excel 批量查询无 N+1：部分完成，自动工号生成仍逐行查询，转 B-06
IMPORT 成功日志 200：通过
...
IMPORT 日志脱敏：通过
导入通知安全摘要：通过
Repository 批量查询测试：通过
Entity/Flyway/init.sql/数据字典一致性：通过，本轮无结构变更
角色写入失败事务回滚：通过
用户管理重复 Toast 回归：通过
完整性诊断日志脱敏：通过
后端测试：119 个测试通过，0 失败
...
## 第二轮代码审计整改补充（2026-06-14）

**当前状态：未完成，待执行。**

### Context

第一轮整改实施并通过自动化测试后，第二轮代码审计进一步发现 6 项仍需处理的安全、契约和
批量性能问题。本节作为当前活动 Plan 的增量整改方案；已归档版本
`docs/plan/done/users-security-stability-audit-remediation-feature.md` 保持不变，用于保留第一轮实施记录。

| 编号 | 风险等级 | 状态 | 问题 | 影响 |
|---|---|---|---|---|
| B-01 | 高 | **已完成** | 管理员重置密码通过 URL 查询参数传输 | 已改为受控 JSON DTO，旧 query 参数不再生效 |
| B-02 | 高 | **已完成** | 用户创建、角色分配和权限解析未统一限制停用角色 | 已统一启用角色校验与权限过滤 |
| B-03 | 中 | **已完成** | 编辑资料与重置密码由前端发起两个独立请求 | 已新增管理员原子编辑接口并切换 PC 页面 |
| B-04 | 中 | **已完成** | `deptId` 使用 `null` 同时表示“不更新”和“清空” | DTO 已支持字段存在性与显式清空语义 |
| B-05 | 中 | **已完成** | 导入行存在其他字段错误时会跳过重复检查 | 无效行中的可用唯一键仍参与完整重复检查 |
| B-06 | 低 | **已完成** | 自动工号生成与导入写入存在逐行查询和逐行 flush | 已改为批量工号检查及批量用户/角色写入 |

### 第二轮目标

1. 所有管理员密码重置请求均通过 JSON 请求体传输，密码不得出现在 URL。
2. 用户创建、用户导入、角色分配、权限解析统一执行启用角色约束。
3. 用户编辑页面提交资料和可选新密码时具备单一事务语义。
4. 用户更新契约能够明确区分字段未提供、设置值和显式清空。
5. 用户导入一次返回所有当前可发现的安全错误，避免重复修复上传。
6. 用户导入预检与写入使用批量数据库操作，并通过查询数量或行为测试验证。

### 范围与非目标

本轮涉及：

- `POST /api/users/{id}/reset-password` 请求契约。
- 新增管理员原子编辑接口及 PC 用户编辑弹窗调用链。
- 用户创建、导入、角色分配和权限解析中的角色状态校验。
- `deptId`、部门名称等可空字段的显式清空语义。
- 用户导入重复检查、批量工号生成和批量写入。
- 对应后端、前端、API、事务和性能回归测试。

本轮不涉及：

- 不修改 `sys_user`、`sys_role`、`sys_user_role` 表结构。
- 不合并“编辑用户”和“分配多角色”功能；多角色仍由独立入口维护。
- 不允许前端自行判断角色是否有效作为唯一安全控制。
- 不将明文密码写入日志、通知、错误响应或审计参数。
- 不改变用户导入“任意错误整批零写入”的语义。

## 第二轮实现方案

### B-01：密码重置改用受控 JSON DTO

新增 `AdminPasswordResetRequest`：
...
- 后端将 `@RequestParam newPassword` 改为 `@Valid @RequestBody AdminPasswordResetRequest`。
- DTO 仅允许 `newPassword`，未知字段返回 HTTP 400。
- 需要使用默认密码时，前端发送空对象 `{}`，后端读取 `security.default-user-password`。
- 前端 `resetUserPassword()` 使用 JSON body，不再构造 query params。
- 操作日志仅记录用户 ID、操作结果和响应码，不记录 DTO 原文。
- Nginx、网关和浏览器地址栏中不得再出现密码。

兼容性决策：这是安全缺陷修复，不保留 URL 参数兼容入口。旧客户端必须同步升级。

### B-02：建立启用角色统一约束

新增或复用统一角色校验服务，例如 `ActiveRoleResolver`，提供：

- 按角色编码获取启用角色，不存在或停用时抛出安全业务异常。
- 批量校验角色 ID：全部存在且状态为 `ACTIVE`。
- 批量过滤用户当前有效角色，供权限计算使用。

各入口规则：

| 入口 | 规则 |
|---|---|
| 创建用户 | 请求中的主角色必须存在且启用；否则用户和角色关联均不得写入 |
| Excel 导入 | 继续使用启用角色映射，写入事务开始后再次确认角色仍启用 |
| 分配多角色 | 所有角色 ID 必须存在且启用；任一无效则保持原角色关联不变 |
| 权限解析 | 仅启用角色可产生权限；停用角色不得继续授权 |
| 主角色回退 | 仅当兼容主角色对应的 `sys_role` 启用时才允许回退 |

`SysRoleService.assignRolesToUser()` 必须先完成全部校验，再删除旧角色，避免无效请求清空现有权限。
用户创建时若角色缺失或停用，必须失败，禁止静默创建仅有 `sys_user.role` 的不完整用户。

### B-03：新增管理员原子编辑入口

保留 `PUT /api/users/{id}` 作为资料与状态的受控更新接口；新增：

```text
PUT /api/users/{id}/admin-edit
```

请求 DTO `AdminUserEditRequest` 包含：
...
服务层新增单一 `@Transactional` 编排方法：

1. 加载目标用户。
2. 校验全部资料字段和可选新密码。
3. 仅在全部校验通过后修改资料及密码。
4. 写入用户、更新 `password_updated_time`、提交事务。
5. 事务提交后刷新通知资格。

PC 用户编辑弹窗仅调用该原子接口，不再先调用 `updateUser()` 再调用 `resetUserPassword()`。
独立“重置密码”入口继续保留，供只重置密码的管理场景使用。

### B-04：可空更新字段采用显式清空语义

普通 Java Bean 无法可靠区分 JSON 字段“未提供”和“显式为 null”。本轮采用字段存在性标记，
避免引入数据库变更：

- `UserUpdateRequest` 与 `AdminUserEditRequest` 对 `department`、`deptId` 等可清空字段使用
  `@JsonSetter` 记录字段是否出现。
- 未提供字段：保持原值。
- 提供非空值：更新为新值。
- 显式提供 `null` 或空白字符串：清空对应字段。
- 前端选择“无部门”时必须发送 `deptId: null`，不得转为 `undefined`。
- 清空 `deptId` 时同时确认是否应清空冗余 `department` 文本；本项目采用两字段分别显式更新，
  防止隐式覆盖。

必须补充序列化/服务测试，验证省略字段不会误清空，显式 `null` 可以清空。

### B-05：导入预检返回全部当前可发现错误

将当前“行校验失败后丢弃该行”的处理拆分为两个结果：

```text
NormalizedImportCandidate
  - 可用的规范化唯一键：username / employeeId
  - 其他规范化字段
  - 行级错误集合
```
...
1. 规范化每一行，并记录字段错误。
2. 只要用户名可用，就参与文件内用户名重复和数据库用户名重复检查。
3. 只要工号可用，就参与文件内工号重复和数据库工号重复检查。
4. 角色、状态、邮箱或密码错误不得阻止该行可用唯一键参与重复检查。
5. 汇总并去重错误后统一返回；存在任一错误时不得编码密码或写入数据库。

错误去重键使用 `rowNumber + field + code`，避免首个重复行被后续每个重复值反复添加相同错误。

### B-06：批量工号生成与批量写入

预检阶段：

- 为缺少工号的行先在内存生成足量候选集合，并排除本批显式工号。
- 使用 `findByEmployeeIdIn()` 批量检查候选集合；存在冲突时仅重新生成冲突数量。
- 设置有限重试轮次，耗尽后返回结构化 `EMPLOYEE_ID_GENERATION_FAILED`，不得抛出无上下文系统异常。
- 禁止每个缺失工号调用一次 `existsByEmployeeId()`。

写入阶段：

1. 写入事务开始后再次批量检查用户名、工号和启用角色状态。
2. 使用 `userRepository.saveAll()` / `saveAllAndFlush()` 批量持久化用户。
3. 根据返回用户 ID 构造全部 `UserRole`，使用 `userRoleRepository.saveAllAndFlush()` 一次写入。
4. 用户和角色关联全部成功后发布资格刷新事件。
5. 任意异常整批回滚。

不以“完全固定 SQL 条数”为目标；验收目标是消除与数据行数线性增长的显式查询和逐行 flush。

## 第二轮前后端与数据库一致性

| 检查项 | 结论与动作 |
|---|---|
| 密码重置 API | 前后端同时从 query 参数改为 JSON DTO；更新 API 手册 |
| 管理员原子编辑 API | 新增 Java DTO、Controller、Service 与 TypeScript 类型/API |
| 用户更新 DTO | Java 与 TypeScript 同步可空字段及显式清空语义 |
| 角色状态 | `sys_role.status` 已存在，无数据库变更；所有角色入口统一使用 |
| 用户导入 | 响应结构不变，错误清单更完整；前端无需改变错误模型 |
| Entity / ORM | 本轮不新增或修改 Entity 字段、表名、列名与关联注解 |
| Flyway / init.sql / 数据字典 | 无结构变更，无需新增 migration；实施后执行一致性检查并记录结果 |

## 第二轮关键参考文件

| 文件 | 用途 |
|---|---|
| `backend/src/main/java/com/pricemanagement/controller/UserController.java` | 密码重置和管理员原子编辑 API |
| `backend/src/main/java/com/pricemanagement/service/UserService.java` | 用户创建、更新、密码和事务编排 |
| `backend/src/main/java/com/pricemanagement/service/SysRoleService.java` | 多角色分配入口 |
| `backend/src/main/java/com/pricemanagement/service/PermissionService.java` | 角色到权限解析 |
| `backend/src/main/java/com/pricemanagement/service/UserImportValidationService.java` | 导入全量错误发现与批量工号预检 |
| `backend/src/main/java/com/pricemanagement/service/UserImportWriteService.java` | 导入批量原子写入 |
| `backend/src/main/java/com/pricemanagement/service/EmployeeIdService.java` | 工号候选生成能力 |
| `frontend/src/api/users.ts` | 密码重置、管理员编辑 API 契约 |
| `frontend/src/views/UserManagement.vue` | 原子编辑、部门清空与错误交互 |
| `docs/dev/项目设计规范.md` | 密码传输、角色状态和原子编辑长期规范 |

## 第二轮实施步骤

### Phase 1：立即修复高风险安全边界

1. 新增 `AdminPasswordResetRequest`，密码重置改用 JSON body。
2. 更新前端 `resetUserPassword()`，确认 URL、日志和错误中均无密码。
3. 建立启用角色统一校验能力。
4. 用户创建、角色分配、权限解析和导入写入接入角色状态校验。
5. 增加停用角色创建、分配和权限隔离测试。

### Phase 2：建立用户编辑原子语义

1. 新增 `AdminUserEditRequest` 与 `/api/users/{id}/admin-edit`。
2. 实现单一事务内资料校验、可选密码校验和持久化。
3. 为可空字段实现字段存在性标记和显式清空。
4. PC 编辑弹窗改用原子编辑接口，并发送 `deptId: null` 清空部门。
5. 增加密码不合规时资料零更新、部门显式清空测试。

### Phase 3：完善导入预检和批量性能

1. 重构行校验结果，保留无效行中仍可检查的用户名和工号。
2. 汇总文件内与数据库重复错误，并按行/字段/错误码去重。
3. 实现批量工号候选生成与批量冲突查询。
4. 写入阶段改用批量用户和角色关联持久化。
5. 保留并扩展真实事务回滚测试。

### Phase 4：文档、回归和归档

1. 更新 API、设计、开发、UI、运维和完成总结文档中实际受影响内容。
2. 执行后端完整测试、前端构建、Flyway validate 和关键人工回归。
3. 记录数据库查询行为、事务回滚和密码不进入 URL 的验证证据。
4. 全部完成后更新本节 Verification，并将活动 Plan 归档。
...
- `UserControllerTests`
  - 密码重置仅接受 JSON body。
  - URL query 中的 `newPassword` 不再生效。
  - 管理员原子编辑参数错误返回 HTTP 400。
- `UserServiceTests`
  - 停用或缺失主角色时创建用户零写入。
  - 原子编辑密码策略失败时资料和密码均不变。
  - 显式 `deptId: null` 清空部门；省略字段保持原值。
- `SysRoleServiceTests`
  - 分配中包含停用/不存在角色时保留原角色关联。
- `PermissionServiceTests`
  - 停用角色不产生权限，兼容主角色回退同样检查启用状态。
- `UserImportValidationServiceTests`
  - 密码或邮箱错误行仍能同时报告用户名/工号重复。
  - 多行重复错误按行、字段、错误码去重。
  - 缺失工号使用批量查询，不逐行调用 `existsByEmployeeId()`。
- `UserImportWriteServiceTransactionTests`
  - 批量写入任意用户或角色失败时整批回滚。
  - 预检后角色被停用时写入失败并零残留。

### 前端验证

- 编辑资料并填写弱密码：资料不得部分保存，页面仅提示一次。
- 选择“无部门”保存后重新打开：部门为空。
- 浏览器 Network、地址栏、服务访问日志：重置密码 URL 不含密码。
- 导入包含“弱密码 + 重复用户名 + 重复工号”的文件：一次展示全部可发现问题。
- 导入 1000 行合法数据：页面可完成导入，用户和角色关联数量一致。

## 第二轮质量门禁

- [x] 密码重置请求 URL 不包含 `newPassword` 或密码值。
- [x] 创建、导入、分配和权限计算均只接受启用角色。
- [x] 角色校验失败不会创建用户或清空原角色。
- [x] 编辑资料与可选新密码为单一事务，任意校验失败零更新。
- [x] 显式 `deptId: null` 可以清空部门，省略字段不改变部门。
- [x] 导入一次返回所有当前可发现的重复与字段错误。
- [x] 导入预检不存在逐行 `existsByEmployeeId()`。
- [x] 导入写入不存在逐行 `saveAndFlush()`。
- [x] 导入批量写入失败整批回滚。
- [x] 操作日志、通知和错误响应均不包含密码。
- [x] Java DTO、TypeScript 类型和 API 文档保持一致。
- [x] 本轮无 Entity / Flyway / init.sql / 数据字典结构变化。
- [x] 完整 `mvn test` 与 `npm run build` 通过。
- [ ] Flyway validate 与关键人工回归通过。

## 第二轮文档更新矩阵

| 文档 | 更新内容 |
|---|---|
| `README.md` | 用户管理安全与原子编辑能力 |
| `docs/dev/开发指南.md` | 密码只能通过请求体传输、启用角色约束、显式清空语义 |
| `docs/dev/项目设计文档.md` | 密码重置和管理员原子编辑 API 契约 |
| `docs/dev/API调用手册.md` | JSON 密码重置、原子编辑请求/响应示例 |
| `docs/dev/UI设计说明.md` | 编辑资料与密码原子提交、无部门清空交互 |
| `docs/dev/项目设计规范.md` | 密码敏感字段传输、角色停用和批量写入长期规范 |
| `docs/archive/项目完成总结.md` | 第二轮安全稳定性整改完成状态 |
| `docs/ops/IDEA部署指南.md` | 发布回归项；本轮无新增配置和数据库 migration |

## 第二轮风险与回滚

| 风险 | 控制与回滚 |
|---|---|
| 密码重置 API 破坏旧客户端 | 前后端同版本发布；发布前检索调用方；不得恢复 URL 密码参数 |
| 停用角色导致现有用户权限立即变化 | 发布前查询停用角色关联用户并形成影响清单；必要时先启用或重新分配角色 |
| 原子编辑新增接口出现兼容问题 | 保留资料更新和独立密码重置接口；PC 页面可回退为仅编辑资料，不回退到非原子双请求 |
| 显式清空误删部门 | DTO 字段存在性测试和人工回归；仅显式 `null` 执行清空 |
| 批量写入改变事件时序 | 仅在用户及角色全部写入成功后发布刷新事件；事务回滚测试验证 |
| 批量工号生成耗尽 | 有限重试并返回结构化错误，不进入写入阶段 |
...
```text
密码重置 JSON DTO：通过（Controller 契约测试）
密码未出现在 URL：通过（前后端与正式文档静态检索）
创建用户启用角色校验：通过
角色分配启用状态校验：通过
停用角色权限隔离：通过（存在停用关联时禁止兼容主角色回退）
管理员编辑原子事务：通过
部门显式清空：通过
导入完整错误清单：通过
导入错误去重：通过
...
批量用户与角色写入：通过
导入事务回滚：通过
敏感日志审查：通过（静态审查；生产访问日志人工核验未执行）
前后端 API 契约：通过
Entity/Flyway/init.sql/数据字典一致性：通过（本轮无结构变更）
后端测试：通过，mvn test 共 131 项，0 failures，0 errors
前端构建：通过，npm run build
git diff --check：通过
Flyway validate：未执行；test profile 明确禁用 Flyway，需连接目标 MySQL 环境验证
浏览器关键人工回归：未执行
数据库变更：无
```
...
- B-01 至 B-06 均有代码修复、自动化测试和验证证据。
- 密码不再通过 URL、日志、通知或错误响应传播。
- 停用角色无法被新增分配且不能继续产生权限。
- 管理员编辑资料和可选密码具备单一事务语义。
- 用户可以显式清空部门关联。
- 导入预检一次返回全部当前可发现问题，合法批次使用批量查询和写入。
- 正式文档与实际 API、前端交互和运维验证方式一致。
