...
- PC 消息入口固定收敛在左侧侧边栏底部用户卡片更多菜单中；有未读消息时三点按钮切换为红色数字提示，Layout 按智能轮询策略刷新 `/api/notifications/unread-count`，路由切换、页面重新可见和价格发布成功后额外刷新。
- 点击通知打开站内通知抽屉，未读通知点击后调用 `/api/notifications/{messageId}/read` 标记已读；价格发布通知跳转价格查询页并带上发布日期。
...
- 为旧正式价格写入接口增加用途注释和权限校验说明；`ADMIN/EDITOR` 保留价格编辑能力，`VIEWER` 禁止写入。
- 新增请求来源或操作场景标识 `source=FORMAL_PRICE_MAINTENANCE`，用于日志和审计识别。
...
| 旧接口权限收口 | 二期 | 已完成 | `ADMIN/EDITOR` 保留价格编辑能力，`VIEWER` 禁止写入 |
| 旧接口请求来源/审计标识 | 二期 | 已完成 | 旧正式价格写入支持 `source=FORMAL_PRICE_MAINTENANCE` 并记录操作日志 |
| 防重复发布幂等策略 | 二期 | 已完成 | 已发布或发布中批次重复发布返回 409，不重复写正式价格 |
