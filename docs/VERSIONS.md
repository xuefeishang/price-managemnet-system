...
| v2.1.0-20260616 | 2026-06-16 | 次版本 | **安全加固 v1.0**：用户管理安全（删公开注册/密码策略/数据脱敏）；nginx 攻击特征 map + 分层限流；iptables 限源 3306/8082；新增 security.md 设计文档；方案 docs/plan/security-hardening-2026-q2.md |
...
## v2.1.1-20260704 (2026-07-04)

### 补丁版本：SSE 客户端断连日志治理

- 修复 `/api/notifications/events` 客户端刷新、关闭页面或前端主动 abort 后，服务端将正常断连记录为 `Unexpected error occurred` 的问题。
- 补齐 `AsyncRequestNotUsableException` 异步生命周期处理，避免 `text/event-stream` 响应已建立后再次写入 `Result<T>` JSON 并触发 `HttpMessageNotWritableException`。
- 调整通知 SSE `connected` 事件发送边界，新订阅只通知当前连接，断开的 emitter 只清理自身，不影响同用户其它连接和轮询兜底。
