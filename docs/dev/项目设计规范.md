...
| `jwtExpiration` | long | `${JWT_EXPIRATION:3600000}` (1h) | accessToken 过期时间（毫秒） |
| `corsAllowedOrigins` | String[] | 见 application.yml | CORS 允许的源 |
...
| `maxLength` | int | 64 | 密码最大长度 |
| `requireUppercase` | boolean | false | 必须包含大写字母 |
| `requireLowercase` | boolean | false | 必须包含小写字母 |
| `requireDigit` | boolean | true | 必须包含数字 |
| `requireSpecial` | boolean | false | 必须包含特殊字符 |
| `specialChars` | String | "!@#$%^&*" | 允许的特殊字符集 |
| `disallowUsername` | boolean | true | 禁止密码等于用户名 |
| `disallowEmployeeId` | boolean | true | 禁止密码等于工号 |
