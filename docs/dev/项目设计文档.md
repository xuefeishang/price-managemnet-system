...
- 产品新增/编辑/删除
- 价格管理（原价、现价、成本价）
- 价格维护工作台：与首页产品列表样式保持一致，支持产品名称搜索、产品类别筛选、分页、每页条数切换和拖拽调整产品顺序；拖拽落位后调用 `/api/products/batch-sort` 更新产品 `sortOrder`，并通过 `product-sort-updated` 事件刷新首页排序；保留当日售价录入/修改/批量保存，并展示预算价格、昨日售价、价格变化和月均价
