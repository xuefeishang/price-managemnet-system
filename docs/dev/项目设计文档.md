...
- 日常价格查询：只读页面 `/price-query`，聚合产品产地、规格、近期价格、年度预算、较预算差额、月均价和最后维护价；支持 VIEWER 按日期、关键字、分类分页查询，列表按“产品名称、近期价格、预算价格、较预算”展示，产地胶囊和规格副信息融入产品名称单元格。近期价格取该产品最后一次正式价格维护的价格，预算价格取查询日期所属年度 `product_annual_budget`，较预算为二者差额。行选中后复用 `/api/products/{productId}/price-trend` 展示 30天/180天/12个月趋势，并可通过 `startDate/endDate` 按自然年查看；趋势接口返回请求范围内完整日期轴，`currentPrice` 取截至每日有效的最后一次正式价格并持续到下一次正式价格或显式到期日，`budgetPrice` 按日期所属年度引用 `product_annual_budget`。进入产品详情时通过 `date` 查询参数传递当前查询日期；导出接口按同一查询条件导出全部匹配数据。
