...
| home_layout | 首页布局配置 | layout_mode→驾驶舱模式, card_columns→卡片列数 |
...
- **8个配置域**
  ...
  - 首页体验：卡片列数、重点走势开关、预警区开关、组件启用排序
  - 首页排序：首页组件上下顺序、产品列表分类顺序、分类内产品顺序；分类顺序复用 `product_category.sort_order`，产品顺序复用 `product.sort_order`，影响产品列表、重点关注指标与重点走势
...
| GET | /api/home/summary | 获取摘要统计（产品数、涨跌统计） |
