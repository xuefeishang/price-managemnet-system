...
### 8. 全局样式管理（仅管理员）
- **响应式配置工作台**
  - 三栏布局（PC）：左侧导航(200px) | 中央配置区 | 右侧预览区(360px)
  - 移动端：横向滚动导航 + 配置区 + 预览抽屉
  - 三层状态模型：serverConfig / draftConfig / appliedConfig
  - 选择即生效，后台自动保存，失败自动回滚
- **8个配置域**
  - 总览：当前方案摘要、健康检查、快捷操作
  - 品牌：系统名称、Logo上传、Logo尺寸（small/medium/large/xlarge）
  - 色彩：色彩方案切换、价格涨跌色、图表色板（9色）
  - 排版：字号预设切换（紧凑/标准/大字体/无障碍）、字体族配置
  - 布局：布局方案切换（顶部导航/左侧导航/驾驶舱/极简）、页面密度
  - 首页体验：卡片列数、趋势图开关、预警区开关、组件启用排序
  - 分类视觉：分类配色、图标配置、光晕透明度、预设配色
  - 版本恢复：历史版本列表、一键回滚
- **技术实现**
...
分类视觉系统为产品分类提供专属的颜色、图标和光晕效果，实现产品卡片的视觉差异化。

### 数据结构

分类视觉配置存储在 `sys_dict` 表中，`category = 'category_visual_config'`，`extraValue` 存储JSON配置：
...
```json
{
  ...
  "darkMode": {
    ...
    "textColor": "#F5E6D3",
    "borderColor": "#165DFF",
    "glowColor": "rgba(232, 200, 158, 0.2)"
  }
}
```
...
| 分类 | 主色调 | 图标 | 设计理念 |
|------|--------|------|----------|
| 黄金类 | #D4A574 | gold_ingot | 金色光泽，高贵典雅 |
| 白银类 | #A8B5C4 | silver_bar | 银白冷调，洁净科技 |
| 铜类 | #B87333 | copper_coil | 铜红暖调，工业质感 |
| 铁矿石 | #8B4513 | iron_ore | 深褐厚重，沉稳可靠 |
| 铝类 | #C0C0C0 | aluminum_block | 银灰轻盈，现代简约 |
| 稀土类 | #8B5CF6 | rare_element | 紫色神秘，科技前沿 |

### 前端组件

| 组件 | 路径 | 说明 |
|------|------|------|
| CategoryIcons | `components/icons/CategoryIcons.vue` | 分类图标组件（6种矿产品SVG图标） |
...
- **边框统一使用深矿蓝(#165DFF)**，分类色仅用于内部填充和图标
