...
#### 16. sys_role_permission（角色权限关联表）
| 字段 | 类型 | 说明 |
|------|------|------|
| role_id | BIGINT | 角色ID |
...
#### 19. sys_api_key_permission（外部 API 授权表）
| 字段 | 类型 | 说明 |
|------|------|------|
| id | BIGINT | 主键 |
...
#### 20. sys_external_api_endpoint（外部 API 端点映射表）
| 字段 | 类型 | 说明 |
|------|------|------|
| id | BIGINT | 主键 |
...
#### 21. sys_api_call_log（外部 API 调用日志表）
| 字段 | 类型 | 说明 |
|------|------|------|
| id | BIGINT | 主键 |
...
#### 22. sys_api_key_operation_log（API Key 管理操作日志表）
| 字段 | 类型 | 说明 |
|------|------|------|
| id | BIGINT | 主键 |
...
- 分类治理与保护
  ...
  - "显示系统配置"开关（仅ADMIN可见）
  - 受保护分类只读展示，编辑需前往样式设置
  - 分类元数据定义（`dictCategoryMeta.ts`）：domain、extraValueMode、editableInDictPage
- extraValue智能渲染
  - 根据分类元数据渲染不同类型
...
| GET | /api/auth/profile | 获取当前用户信息，返回用户基础字段、动态角色列表 `roles`、主角色 `role` 与权限码集合 `permissions`；历史用户缺少 `sys_user_role` 关联时回退 `sys_user.role` |
