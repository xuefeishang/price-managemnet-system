...
| GET | /api/notifications/events | **SSE 长连接**（详见 v1.6.8 模块 2） |
...
| GET | /api/admin/notifications | 管理员列表（仅 ADMIN） |
| GET | /api/admin/notifications/dashboard | 通知管理仪表盘 |
| GET | /api/admin/notifications/providers/health | Provider 健康检查 |
| GET | /api/admin/notifications/throttle-rules | 频控规则 |
| POST | /api/admin/system-notices | 发布系统公告 |
...
| PUT | /api/scheduled-tasks/{id}/enable | 启用 |
| PUT | /api/scheduled-tasks/{id}/disable | 停用 |
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | /api/price-drafts/by-date | 按日期查询草稿 |
| GET | /api/price-drafts/publishable-summary | 可发布摘要 |
| POST | /api/price-drafts/batch-save | 批量保存草稿 |
| POST | /api/price-drafts/publish-all | **一键发布全系统草稿**（推荐入口）|
| POST | /api/price-drafts/{batchId}/publish | 单批次发布（兼容/维护）|
| POST | /api/price-drafts/by-date/publish | 按日期发布（定时任务/补发）|
| POST | /api/price-drafts/{batchId}/cancel | 取消批次 |

### 外部 API 控制器（v1.3.0+，API Key + HMAC-SHA256 认证）
