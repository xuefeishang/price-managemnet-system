...
- 价格保存/发布：价格维护页先调用 `/api/price-drafts/batch-save` 保存草稿，草稿对其他用户不可见；发布调用 `/api/price-drafts/{batchId}/publish`，成功后写入正式 `price`、`price_history`、同步产品售价并生成站内通知。`PRICE_PUBLISH` 审批能力仅预留，当前不启用。
