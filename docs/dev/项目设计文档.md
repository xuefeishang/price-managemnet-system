...
#### 26. notification_message / notification_recipient / notification_delivery_log / notification_outbox / notification_preference / notification_mini_program_subscription / notification_mini_program_eligibility / notification_channel_config（通知中心表）
| 表 | 关键字段 | 说明 |
|------|----------|------|
...
| notification_mini_program_subscription | user_id、openid、notification_type、template_id、status、available_count、last_authorized_time、source、created_time、updated_time | 小程序订阅消息授权表；用户在小程序端点击授权后写入，`available_count` 按微信一次性订阅授权累计，`MINI_PROGRAM` Provider 发送成功后扣减 |
...
| notification_channel_config | channel、enabled、app_id、endpoint_url、secret_cipher、secret_key_version、secret_fingerprint、timeout_ms、default_page、config_json、created_by、updated_by、version | 通知渠道运行配置表；PC `/notifications` 渠道配置页维护小程序 AppID、启用状态、模板映射、跳转页和密钥托管状态，Provider 发送时优先读取数据库配置，缺失再回退环境变量 |

小程序消息发布规则：PC `/notifications` 是唯一信息发布管理台。管理员创建系统公告并选择 `MINI_PROGRAM` 渠道后，后端仍强制生成 `IN_APP` 站内消息作为可靠兜底；`MINI_PROGRAM` 投递通过 Outbox 异步调用微信订阅消息接口，Provider 未配置、用户未绑定 openid、模板未授权或微信接口拒收均写入 `notification_delivery_log`，不回滚公告发布。
...
- 产品详情查看：响应式详情页优先展示产品身份、当前售价、预算价格与近 30 日变化；页头日期控件调用 `/api/products/{productId}/price-by-date` 查看历史快照和昨日/上周/上月/去年同期基准价，趋势区调用 `/api/products/{productId}/price-trend?days={days}&endDate={yyyy-MM-dd}` 展示截至基准日的售价曲线、预算水平基准线和坐标轴，并按同一基准日范围计算最低价、最高价、平均价、预算价、最新价；对比区展示日环比、周环比、月环比和同比
- 产品新增/编辑/删除
- 价格管理（原价、现价、成本价）
...
- 日常价格查询：只读页面 `/price-query`，聚合产品产地、规格、当日价格、昨日价格、月均价、预算价和最近有效价；支持 VIEWER 按日期、关键字、分类分页查询，列表按“产品名称、当日售价、昨日售价、较昨日”展示，产地胶囊和规格副信息融入产品名称单元格，行选中后复用 `/api/products/{productId}/price-trend` 展示趋势；进入产品详情时通过 `date` 查询参数传递当前查询日期，确保详情历史快照与查询上下文一致；导出接口按同一查询条件导出全部匹配数据
- 小程序角色化移动闭环：`frontend-uniapp` 以 PC 端能力为基础做移动任务化，VIEWER 底部导航为“首页 / 历史 / 我的”，ADMIN/EDITOR 增加“录入”；`pages/history/index` 提供只读历史价格查询，`pages/price-maintenance/index` 承载轻量价格录入/补录，并复用 `/api/price-drafts/batch-save` 与 `/api/price-drafts/{batchId}/publish` 实现“保存草稿 → 发布生效并通知”的双按钮流程；小程序通过 `/api/notifications/unread-count` 轮询共享未读状态，价格发布成功或检测到新通知时展示动态气泡，并在自定义底栏“我的”入口显示红色数字角标；小程序首页重点关注与产品列表复用 PC 首页的分类 `sortOrder` + 产品 `sortOrder` 稳定排序，并从币种字典读取产品货币符号；产品、分类、产地、客户、审批、字典、用户、菜单、日志、样式等完整运维保留在 PC 端，不作为小程序主导航入口
- 价格历史记录追踪
- Excel批量导入导出
...
- 权限边界：通知管理接口使用 `notification:view`、`notification:retry`、`system-notice:create`、`system-notice:cancel`、`system:setting` 进行按钮级 API 授权；渠道配置保存需要 `system:setting`，JWT 中携带权限码并由 `JwtAuthenticationFilter` 注入 Spring Security authorities。
- 小程序配置运维：`MINI_PROGRAM` Provider 通过 `NotificationMiniProgramRuntimeConfigService` 解析运行配置，优先读取 `notification_channel_config`，数据库未配置时回退 `notification.mini-program.*` 环境配置。AppSecret 使用 `ApiKeySecretService` AES-GCM 加密存储，前端响应只返回“已托管/未配置”和脱敏 AppID/模板信息。
- 小程序资格分页：`NotificationMiniProgramEligibilityService` 维护用户级资格快照；订阅授权、额度消费、用户创建/更新事务提交后触发单用户刷新，模板配置变化和每日低峰任务执行全量校准。管理列表带 `status` 筛选时通过 `sys_user` JOIN 快照表分页，当前页明细仍读取原始授权记录。
...
| GET | /api/admin/notifications/channels/{channel}/config | ADMIN | 查询渠道配置详情；当前支持 `MINI_PROGRAM`，返回脱敏 AppID、密钥状态、模板映射和诊断清单 |
| PUT | /api/admin/notifications/channels/{channel}/config | ADMIN | 保存渠道配置；需要 `system:setting`，AppSecret 只写入密文不回显 |
