...
const handleToggleStatus = async (origin: Origin) => {
  ...
  try {
    ...
    await updateOrigin(origin.id, { status: newStatus } as Partial<Origin>)
    origin.status = newStatus
    showToast(`产地已${actionText}`)
  } catch (error) {
...
const handleEdit = (origin: Origin) => {
  router.push(`/origin-edit/${origin.id}`)
}

const handleCreate = () => {
  router.push('/origin-edit')
}

const handleDelete = async (origin: Origin) => {
  showDialog({
    title: '确认删除',
    message: `确定要删除产地"${origin.name}"吗？删除后不可恢复。`,
...
<template>
  <div class="origins-page">
    ...
    <template v-if="isPCLayout">
      <div class="pc-origins">
        <div class="page-header-pc">
          <h1 class="page-title-pc">产地管理</h1>
        ...
        <div class="origin-table-pc" v-if="!loading">
          <div class="table-header">
            <div class="table-cell name">产地名称</div>
            <div class="table-cell code">产地编码</div>
          ...
          <div
            ...
            :key="origin.id"
            class="table-row"
            :class="{ inactive: origin.status === 'INACTIVE' }"
          >
            <div class="table-cell name">
              <div class="origin-name-cell">
                <div class="origin-icon" :class="origin.status">
                  <svg v-if="origin.status === 'ACTIVE'" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            ...
            <div class="table-cell code">{{ origin.code }}</div>
            <div class="table-cell status">
              <div
                class="toggle-switch"
                :class="{ active: origin.status === 'ACTIVE', loading: togglingId === origin.id }"
                @click="handleToggleStatus(origin)"
                :title="origin.status === 'ACTIVE' ? '点击停用' : '点击启用'"
              >
            ...
            <div class="table-cell remark">{{ origin.remark || '-' }}</div>
            <div class="table-cell actions" @click.stop>
              <button class="action-btn edit" @click="handleEdit(origin)">
              ...
              <button class="action-btn delete" @click="handleDelete(origin)">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          ...
          <div v-if="origins.length === 0" class="empty-state-pc">
    ...
    <template v-else>
      ...
      <main class="content">
        <div class="origin-list" v-if="!loading && origins.length > 0">
          <div
            v-for="origin in origins"
            :key="origin.id"
            class="origin-card"
          ...
          >
            <div class="card-main">
              <div class="origin-icon" :class="origin.status">
                <svg v-if="origin.status === 'ACTIVE'" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                  <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/>
                  <circle cx="12" cy="10" r="3"/>
                </svg>
              ...
              <div class="card-info">
                ...
                <div class="card-meta">
                  <span class="card-code">{{ origin.code }}</span>
                  <span class="separator">·</span>
                  <span>排序 {{ origin.sortOrder || 0 }}</span>
                </div>
              </div>
            </div>
            ...
            <div class="card-control">
              ...
              <div class="card-actions">
                <button class="card-btn edit" @click="handleEdit(origin)">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/>
                    <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/>
                  </svg>
                </button>
                <button class="card-btn delete" @click="handleDelete(origin)">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    <polyline points="3 6 5 6 21 6"/>
                    <path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/>
                  </svg>
            ...
            <div class="card-remark" v-if="origin.remark">
              {{ origin.remark }}
            </div>
          </div>
        </div>
...
.origin-card.inactive {
  background: #FAFAFA;
}

.origin-card.inactive .origin-icon {
  background: #E5E7EB;
  color: #9CA3AF;
}

.origin-card.inactive .card-name {
  color: #9CA3AF;
}
