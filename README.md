...
- **用户认证与权限管理**
  ...
  - 管理员用户管理：新增、编辑资料、锁定/解锁、删除、导入导出、独立多角色分配；关闭公开注册入口，新增/更新用户使用受控请求字段；Excel 导入先完成模板、字段、文件内重复和数据库重复全量预检，仅在无异常时原子写入
...
- **产品与价格管理**
  ...
  - 首页日期选择，查看历史价格；产品列表较上期差异与价格查询页“较上期差异额”保持一致
  - 产品分类管理
  - 产品与价格 CRUD
  ...
  - 价格维护支持“保存草稿 / 发布全部草稿”双阶段：保存只写入当前日期草稿；PC 与小程序发布按钮会发布全系统所有 `DRAFT` 价格草稿，不受当前页面日期限制，并写入正式价格、价格历史、同步产品售价并生成一条发布组通知；发布重试跳过已发布明细；按日期发布接口仅保留给定时任务、补发或维护场景；PC 左侧用户卡片更多菜单提供消息入口，未读时三点按钮切换为红色数字角标，通知抽屉支持未读筛选、全部已读、归档和价格查询结构化跳转；预留 `PRICE_PUBLISH` 审批扩展点但默认不启用
  - 价格查询页面向普通用户开放，只读查询价格，支持按日期、关键字、分类分页筛选；列表展示产品名称（内含产地胶囊与规格）、近期价格、预算价格和“较预算”，行选中联动右侧 30天/180天/12个月趋势及紧凑三列价格指标洞察；指标以各产品最新有效价格日为统计基准，分组、名称、排序、启停和说明由字典管理统一维护
...
- **多端支持（uni-app）**
  ...
  - 小程序登录页环境切换仅保留“正式地址 / 自定义”，正式地址不展示具体网址，自定义可填写网址和端口
  - 小程序入口分享：首页支持转发好友/微信群和右上角分享到朋友圈；登录页和我的页支持转发小程序入口；分享标题跟随 PC 端系统名称，封面使用公司 Logo，不携带产品、价格或 token
...
### 开发文档 (`docs/dev/`)
v2.0 结构（2026-06-15 起），按 CLAUDE.md 单一职责原则拆分。详见 [docs/dev/README.md](docs/dev/README.md)。

| 文档 | 说明 |
|------|------|
| [docs/dev/README.md](docs/dev/README.md) | 开发文档总览与索引 |
| [docs/dev/stack.md](docs/dev/stack.md) | 前后端技术栈 |
| [docs/dev/coding-standards.md](docs/dev/coding-standards.md) | 代码规范 |
| [docs/dev/api/](docs/dev/api/README.md) | API 文档（README + specs + auth + internal + external）|
| [docs/dev/design/](docs/dev/design/README.md) | 设计文档（架构 + 数据库 + UI + 规范）|
| [docs/dev/workflow/](docs/dev/workflow/README.md) | 工作流（开发 + Git + 部署 + 学习路径）|
...
- 管理员重置密码使用 JSON 请求体，密码不会进入 URL。
- 编辑资料与可选新密码采用单一事务，并支持 `deptId: null` 显式清空部门。
- 创建、导入、角色分配和权限计算只接受启用角色。
- Excel 导入先返回完整错误清单，全部通过后批量原子写入用户及角色关联。
